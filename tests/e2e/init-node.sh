...
# Set client config
nxqd config keyring-backend "$KEYRING"
nxqd config chain-id "$CHAINID"

# if $KEY exists it should be deleted
nxqd keys add "$KEY" --keyring-backend $KEYRING --algo "$KEYALGO"

# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
nxqd init "$MONIKER" --chain-id "$CHAINID"
...
node_address=$(nxqd keys list | grep  "address: " | cut -c12-)
...
nxqd add-genesis-account $KEY 100000000000000000000000000aevmos --keyring-backend $KEYRING
...
nxqd gentx $KEY 1000000000000000000000aevmos --keyring-backend $KEYRING --chain-id "$CHAINID"
## In case you want to create multiple validators at genesis
## 1. Back to `nxqd keys add` step, init more keys
## 2. Back to `nxqd add-genesis-account` step, add balance for those
## 3. Clone this ~/.nxqd home directory into some others, let's say `~/.clonedEvmosd`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedEvmosd/config/gentx/` folders into the original `~/.nxqd/config/gentx`

# Collect genesis tx
nxqd collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
nxqd validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
nxqd start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aevmos --json-rpc.enable true --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable
