...
## default port prefixes for nxqd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/nxqd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/nxqd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/nxqd add-genesis-account \
    "$("$PWD"/build/nxqd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aevmos,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/nxqd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/nxqd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/nxqd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/nxqd start --pruning=nothing --rpc.unsafe \
